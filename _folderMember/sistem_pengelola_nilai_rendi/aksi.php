...
if( isset($aksi) && $aksi == "delete" ){
    ...
    $qry = "DESC $table";
    $dataTable = mysqli_query($conn, $qry);
...
if( isset($aksi) && $aksi == "acakPass" ){
    ...
    mysqli_query($conn, "UPDATE $table SET password='$passwordAcak' WHERE id='$id'");
...
if( isset($aksi) && $aksi == "tambah"){
    ...
    if($table == "mapel"){
        $namaMapel = $_POST['mapel'];
        $row = mysqli_query($conn, "SELECT * FROM $table WHERE nama_m='$namaMapel'");
    ...
    if($table == "mengajar"){
        ...
        $dataGuru = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM guru WHERE id = '$id_guru'"));

        $nama_m = $_POST['nama_m'];
        
        $dbSiswa = mysqli_query($conn, $querySiswa);
        $dataSiswa = [];
        ...
        for($i=0;$i<$jumlahData;$i++){
            ...
            mysqli_query($conn, $queryMengajar);
            ...
            $resultMengajar = mysqli_query($conn, $queryDataMengajar);
